...
def reverse_cat(x):
    num, l_x = x.shape[0], x.shape[1]
    x_new, x_new_reverse = [], []
    for i in range(num):
        x_one = x[i, :]
        x_one_reverse = np.flip(x_one)
        x_new.append(x_one)
        x_new_reverse.append(x_one_reverse)
    x_new = np.expand_dims(np.array(x_new), axis=0)
    x_new_reverse = np.expand_dims(np.array(x_new_reverse), axis=0)
    x_new = np.concatenate((x_new, x_new_reverse), axis=0)
    return x_new
...
ts_name = "HadCRUT5_southern"       # Name of the selected time series
...
cal.plot_spiral(plot_predict, fig_size)           # predict results in the coming year
if save_fig:
    plt.savefig(osp.join(result_address, "future_predict.png"))
cal.plot_spiral(plot_true, fig_size)              # true data in the coming year
if save_fig:
